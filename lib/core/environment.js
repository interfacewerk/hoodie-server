...
var is_in_app = function(project_dir) {
  return project_dir.indexOf('hoodie-server') === -1;
};
...
exports.getConfig = function (platform, env, cwd, argv) {
  ...
  var env_config = {
    ...
    auth_token: env.HOODIE_AUTH_TOKEN || 'secret', 
    app: pkgfile,
    domain: 'dev',
    verbose: false,
    local_tld: undefined,
  ...
  if (argv.hasOwnProperty('auth-token')) {
    env_config.auth_token = (argv['auth-token']);
  }
