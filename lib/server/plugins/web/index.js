var helpers = {
	...
	authenticateRequest: function (options, request, success_path, failure_path) {
		...
		if (headers.length === 2 && headers[1] === options.app.auth_token || remote_ip.substr(0, 4) === '127.' || remote_ip.substr(0, 8) === '192.168.' || remote_ip.substr(0, 3) === '10.' || remote_ip.substr(0,11) === '::ffff:127.') {
			return success_path;
		} else {
			return failure_path;
		}
